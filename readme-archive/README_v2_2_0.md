[README.md](https://github.com/user-attachments/files/32495081/README.md)
# ConnectivityOverlay
...
ConnectivityOverlay supports up to ten ping targets.
...
## 3.4 Fixed Default Targets: VPN-Cisco, VPN-Global Protect, Microsoft MyApps, XiFin SSO

Four fixed-identity targets ship with the application, each pointed at a specific default hostname:

| Target | Default hostname | Enabled by default |
|---|---|---|
| VPN-Cisco | `vpn.xifin.com` | Yes |
| VPN-Global Protect | `connect.xifin.com` | Yes |
| Microsoft MyApps | `myapps.microsoft.com` | Yes |
| XiFin SSO | `sso.xifin.net` | Yes |

These targets are not user-renamable (unlike Custom1-3 below), but their address is: each has a mode switch (`IP` / `URL`) plus one value box. Whichever mode is selected is what the user types; the other side (IP or URL) is resolved automatically via DNS lookup and cached, so the two can never be entered independently and disagree with each other. Each also has an independent graph color and graph-visibility checkbox, exactly like the other targets.
...
- A mode switch (`IP` / `URL`) plus one value box — the other side is resolved automatically via DNS lookup and cached, the same mechanism used by the fixed default targets above.
...
Every target row (including GW, 1st Pub Hop, and DNS) has a small `T` button that opens a live traceroute to that target in a separate small window positioned to the left of the main overlay. Hops render as they resolve rather than after the whole route finishes. Up to 3 traceroute windows can be open at once, lining up side by side so different targets can be compared directly; opening a 4th closes the oldest. Each traceroute window has Copy All, Write to Disk (never the main application log), Refresh (disabled while a trace is running), and Cancel (distinct from the window's own close button).
...
- VPN-Cisco/VPN-Global Protect/Microsoft MyApps/XiFin SSO/Custom1/2/3 continue to use Primary.
- GW/1st Pub Hop/DNS each get their own inline legend directly above their own row whenever their thresholds actually differ from Primary; the shared Primary legend moves down to sit above the fixed/custom-target section instead of the header row, since Primary is what governs that section.
...
VPN-Cisco:  <hostname>    <ip>  <latency>
VPN-Global Protect: ...
Microsoft MyApps: ...
XiFin SSO: ...
----------------
Custom1/2/3 (same 3-column layout, using each target's own name/URL)
----------------
  [Copy]  [^^]
```
...
- DNS (auto-detected) and custom ping targets, with URL or IP entry and automatic DNS resolution.
- Fixed default targets for common enterprise access paths (VPN, Microsoft MyApps, XiFin SSO).
- Per-target Traceroute with live-updating hops.
- Configurable latency-health profiles.
- Advanced per-target latency thresholds, with independent legends.
...
- Redesigned, bordered Compact mode with one-click Copy.
